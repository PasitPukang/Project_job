...
export default function RegisterSeeker() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // บันทึกข้อมูล User ลง LocalStorage และไปยังหน้าหลัก
  ...
  const loginWithGoogle = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        setLoading(true);
        // เรียก API หลังบ้านของเราเพื่อตรวจสอบ Google token และ Login/Register
  ...
  return (
    <div className="max-w-[1000px] mx-auto px-4 py-12 flex flex-col md:flex-row items-center gap-16 min-h-[calc(100vh-80px-200px)]">
      ...
      <div className="w-full md:w-[450px] bg-[#e8ecf1] rounded-2xl p-8 shadow-sm">
        <h1 className="text-2xl font-bold text-[#1e3a8a] text-center mb-2">{t('registerTitleSeeker')}</h1>
        <p className="text-sm text-gray-700 text-center mb-6">{t('seekerRegisterSubtitle')}</p>
        ...
        <form className="space-y-4" onSubmit={handleRegister}>
          ...
          <div className="pt-4 flex flex-col items-center gap-3">
            ...
            <div className="text-xs text-gray-600">
              ...
              <Link to="/login/seeker" className="text-blue-600 hover:underline">
                {t('loginHere')}
              </Link>
            </div>
          </div>
