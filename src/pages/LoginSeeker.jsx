...
export default function LoginSeeker() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || t('loginErrorText'));
      }
  ...
  return (
    <div className="max-w-[1000px] mx-auto px-4 py-12 flex flex-col md:flex-row items-center gap-16 min-h-[calc(100vh-80px-200px)]">
      ...
      {/* Left Form Box */}
      <div className="w-full md:w-[400px] bg-[#e8ecf1] rounded-2xl p-8 shadow-sm">
        <h1 className="text-2xl font-bold text-[#1e3a8a] text-center mb-2">{t('loginTitleSeeker')}</h1>
        <p className="text-sm text-gray-700 text-center mb-6">{t('seekerLoginSubtitle')}</p>
        ...
        <form className="space-y-4" onSubmit={handleLogin}>
          ...
          <div className="flex justify-between items-center text-xs text-gray-700">
            ...
            <a href="#" className="hover:text-blue-700">{t('forgotPassword')}</a>
          </div>
          ...
          <div className="pt-4 flex flex-col items-center gap-3">
            ...
            <div className="text-xs text-gray-600">
              {t('dontHaveAccount')}{' '}
              <Link to="/register/seeker" className="text-blue-600 hover:underline">
                {t('registerHere')}
              </Link>
            </div>
          </div>
        ...
        <div className="mt-6 relative">
          ...
          <div className="relative flex justify-center text-xs">
            <span className="px-2 bg-[#e8ecf1] text-gray-700 font-medium">{t('orLoginWith')}</span>
          </div>
        </div>
      ...
      <div className="w-full md:w-[500px]">
        ...
        <h2 className="text-xl font-bold mb-4 text-black">
          {language === 'th' ? 'คำแนะนำในการตั้งรหัสผ่าน :' : 'Password Recommendations:'}
        </h2>
        <ul className="space-y-2 text-sm text-gray-800 list-disc list-inside">
          <li>{t('passwordAdvice1')}</li>
          <li>{t('passwordAdvice2')}</li>
          <li>{t('passwordAdvice3')}</li>
          <li>{t('passwordAdvice4')}</li>
        </ul>
      </div>
